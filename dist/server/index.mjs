...
function createValidation(config2) {
  function validate(_ref, cb) {
    ...
    function createError(overrides = {}) {
      ...
      const error2 = new ValidationError$2(ValidationError$2.formatError(overrides.message || message, nextParams), value, nextParams.path, overrides.type || name2);
...
(function(module) {
  ...
  function populateConstructorExports(exports2, codes2, HttpError) {
    codes2.forEach(function forEachCode(code) {
      var CodeError;
      var name2 = toIdentifier2(statuses$1.message[code]);
...
const normalizeSignal = function({
  ...
  return { name: name2, number: number2, description: description2, supported, action, forced, standard };
};
...
const getSignalByName = function(signalByNameMemo, { name: name2, number: number2, description: description2, supported, action, forced, standard }) {
  return {
    ...signalByNameMemo,
    [name2]: { name: name2, number: number2, description: description2, supported, action, forced, standard }
  };
};
...
const findSignalByNumber = function(number2, signals) {
  const signal = signals.find(({ name: name2 }) => _os.constants.signals[name2] === number2);
  if (signal !== void 0) {
    return signal;
  }
...
const RemoteSelectFetchOptionsSchema = create$1().shape({
  fetch: create$1().shape({
    url: create$2().required(),
    headers: create$2().optional(),
    body: create$2().optional()
  }),
  mapping: create$1().shape({
    sourceJsonPath: create$2().required(),
    valueJsonPath: create$2().optional(),
    labelJsonPath: create$2().optional()
  })
});
const { ValidationError: ValidationError3 } = errors;
const FetchOptionsProxyController = ({ strapi: strapi2 }) => ({
  async index(ctx) {
    try {
      ...
      ctx.body = await strapi2.plugin("remote-select").service("OptionsProxyService").getOptionsByConfig(flexibleSelectConfig);
    } catch (error2) {
      if (error2.name === "ValidationError")
        throw new ValidationError3("Validation error", error2.errors);
      throw error2;
    }
  }
});
...
const OptionsProxyService = ({ strapi: strapi2 }) => ({
  /**
   ...
   */
  async getOptionsByConfig(config2) {
    const res = await fetch(this.replaceVariables(config2.fetch.url), {
      method: config2.fetch.method,
      headers: this.parseStringHeaders(config2.fetch.headers),
  ...
  parseStringHeaders(headers) {
    ...
    for (let i = 0; i < headersArr.length; i++) {
      ...
      const index2 = row.indexOf(":"), key = this.trim(row.slice(0, index2)).toLowerCase(), value = this.trim(row.slice(index2 + 1));
  ...
  replaceVariables(str2) {
    const variables = strapi2.config.get("plugin.remote-select")?.variables ?? {};
    if (!str2 || typeof str2 !== "string") {
      return str2;
    }
    return str2.replace(/\{([^}]+)\}/g, (match, key) => {
